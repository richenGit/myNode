...
(function (win, doc) {
    // 压缩代码相关
    /* compressor */
    var alogVersion = '1.0.0';
    var objectName = win.alogObjectName || 'alog';
    var oldObject = win[objectName];
    if (oldObject && oldObject.defined) { // 避免重复加载 entry.defined;
        return;
    }
    /*<ie>*/
    ...
    function entry(params) {
        // console.log('71 entry', params);//pv.create
        // console.log('72 arguments', arguments);
        var args = arguments;
        var moduleName;
        var requires;
        var creator;
        ...
        String(params).replace(/^(?:([\w$_]+)\.)?(\w+)$/,
            function (all, trackerName, method) {
                //console.log('trackerName', trackerName, method);
                args[0] = method; // 'hunter.send' -> 'send'
                command.apply(entry.tracker(trackerName), args);//tracker:getTracker();
            }
        );
    }
    /**
     ...
     * 默认追踪器
     */
    var defaultTracker;//调用了getTracker()
    /**
     * 加载模块
     *
    ...
    function loadModules(moduleName) {
        ...
        if (loadScripts[scriptUrl]) {//预防重复加载
            return;
        }
    ...
    function clearDeps(module) {
        if (module.defined) {//避免多次依赖
            return;
        }
        var defined = true;
        var params = [];
        var requires = module.requires;//[speed]
        if (requires) {
            for (var i = 0; i < requires.length; i++) {
                var moduleName = requires[i];
                var deps = modules[moduleName] = (modules[moduleName] || {});//modules['speed']
    ...
    function fire(eventName) {
        var listener = alogListeners[eventName];
        //console.log('触发事件',eventName,listener);
        if (!listener) {
            return;
        }
        var items = [];
    ...
    function Tracker(name) {
        console.log('Tracker追踪器构造器', name);
        this.name = name;
        this.fields = {
            protocolParameter: {
                postUrl: null,
    ...
    Tracker.prototype.send = function (hitType, fieldObject) {
        //console.log('发送日志数据', hitType, this.fields, fieldObject);
        var data = merge({
    ...
    Tracker.prototype.fire = function (eventName) {
        var items = [this.name + '.' + eventName];  //pv + send
        var args = arguments;
        for (var i = 1, len = args.length; i < len; i++) {
            items.push(args[i]);
        }
        return fire.apply(this, items);//执行fire方法 将fire里的this指向为为参数第一个
    };
    /**
     * 绑定事件
     *
    ...
    if (oldObject) {//处理执行缓存的命令 入口是entry
        // 处理临时alog对象
        var items = [].concat(oldObject.p || [], oldObject.q || []);
        oldObject.p = oldObject.q = null; // 清理内存,清掉所有缓存的指令
        for (var p in entry) {
    ...
    win[objectName] = entry;//替换成为全局入口 window['alog']= entry
    /*<ie>*/
    if (ie) {
